...
def sample_nested_sampling(log_likelihood, prior, n_live, tol=0.01, n_max_iter=100000):
    ...
    def log_likelihoods(points):
        return np.apply_along_axis(log_likelihood, 1, points)
    ...
    log_L = log_likelihoods(live_points)
    ...
    while i < n_max_iter:
        ...
        while not drain_live_points:
            ...
            log_L_new = log_likelihoods(new_point[np.newaxis, :])[0]  # Ensure it's 2D for the likelihood function
            n_eval += 1
