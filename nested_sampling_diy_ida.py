...
def sample_nested_sampling(log_likelihood, prior, n_live,
    ...
    live_points = prior.rvs(n_live)  ###rvs - random variates sampling, generates n_live live points from prior
...
def log_likelihood(points):
    return -np.array([himmelblau(point) for point in points])


prior = uniform(0., 2.)

log_Z, dead_points, weights = sample_nested_sampling(log_likelihood, prior=prior, n_live=100, tol=0.01, n_max_iter=10000)